...
export const DataTable: React.FC<DataTableProps> = ({ data, sortConfig, onSort }) => {
  ...
  return (
    <>
      ...
      <div className="w-full rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm bg-white dark:bg-gray-900 flex flex-col h-full">
        <div className="overflow-auto custom-scrollbar flex-1">
          <table className="w-full text-sm text-left border-collapse relative">
            <thead className="bg-gray-50 dark:bg-gray-800/80 text-gray-700 dark:text-gray-300 uppercase text-xs font-semibold sticky top-0 z-10 backdrop-blur-sm shadow-sm">
              <tr>
                {data.headers.map((header) => (
                  <th
                    key={header}
                    scope="col"
                    className={`whitespace-nowrap cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors select-none group border-b border-gray-200 dark:border-gray-700 ${header === 'link' || header === 'links' ? 'w-px px-2' : 'px-6 py-4'}`}
                    onClick={() => onSort(header)}
                  >
                    <div className={`flex items-center gap-2 ${header === 'link' || header === 'links' ? 'justify-start' : ''}`}>
                      {header}
                      <span className="flex flex-col text-gray-400 w-3">
                        {sortConfig.key === header && sortConfig.direction === 'asc' ? (
                          ...
                          <div className="opacity-0 group-hover:opacity-50 flex flex-col">
                             <ChevronUpIcon />
                          </div>
                        )}
                      </span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="h-24 w-auto overflow-hidden rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 cursor-pointer group">
                           <img 
                             src={row[header].startsWith('http') ? row[header] : `data/pics/${row[header]}`} 
                             alt={row['name'] || 'Image'} 
                             className="h-full w-auto object-cover group-hover:scale-105 transition-transform duration-300"
                             onClick={() => setPreviewImage(row[header].startsWith('http') ? row[header] : `data/pics/${row[header]}`)}
                             onError={(e) => {
